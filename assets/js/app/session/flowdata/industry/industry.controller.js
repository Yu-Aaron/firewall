(function () {
    ...
    angular
        ...
        .controller('ICDeviceDataCtrl2', ICDeviceDataCtrl2)
        .controller('ICDeviceProtocolTrafficDataCtrl2', ICDeviceProtocolTrafficDataCtrl2);

    function ICDeviceDataCtrl2(icdevicedata, apiInfo, $scope, $q, $interval, trafficDataService) {
                ...
                return apiInfo.sysbaseinfo().then(function (data) {
                    ...
                    for (var j = 0; j < devices.length && j <= maxLineCount; j++) {
                        ...
                        if (isNewSeries) {
                            chart.addSeries({
                                id: device.deviceId,
                                name: (device.Id === undefined || device.Id === null || device.Id === '0' || device.Id === '0') ? device.deviceName + " (" + device.deviceId + ")" : device.deviceName,
                                color: getFirstAvailableColor()
                            });
                        } else {
                            deviceSeries.update({name: (device.Id === undefined || device.Id === null || device.Id === '0' || device.Id === '0') ? device.deviceName + " (" + device.deviceId + ")" : device.deviceName});
                        }
    ...
    function ICDeviceProtocolTrafficDataCtrl2(deviceId, deviceInfo, icdevicedata, apiInfo, $scope, $q, $interval, trafficDataService) {
