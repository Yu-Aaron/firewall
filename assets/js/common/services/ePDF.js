...
(function () {
    ...
    function ePDF($q, $rootScope, $http, $timeout) {
        //==============Default Setting:================
        var setting = {
            fileName: "Report",
            title: "",
        ...
        function getPdf(options, fileType) {
            ...
            //if (!options.fieldName || !options.fieldName.length) {
            ...
            //var datwa = convertData(options.data, options.fieldName, options.fieldIndex);
            //Apply Options:
            ...
            setting.totalPageNum = (options.img && options.img.length ? options.img.length : 0);
            ...
            convertAllTable(null, (options.img) ? (setting.yOffset + setting.chartH * options.img.length + setting.lineH * options.img.length) : setting.yOffset, setting, doc, setting.title, pageIndex, convertedImg).then(function () {
                if (fileType === "html") {
                    if (options.img) {
                        for (var i = 0; i < options.img.length; i++) {
                            htmlConvertData = htmlConvertData.replace('<table class="wPDF-table"', '<div style="text-align:center;"><img style="width:1000px;height:350px" src="' + convertedImg[i] + '"><br /><br /><table class="wPDF-table"');
                        }
                    }
        ...
        function renderSinglePage(data, y, doc, title) {
            ...
            if (data && data.length && doc) {
                ...
                if (title) {
                    title = '<div style="margin: 0 auto; width:' + setting.fullWidth + 'px;"><h2 style="text-align:center; width:100%;">' + title + '</h2></div>';
                }
                else {
                    title = '';
                }
        ...
        function convertAllTable(d, firstPageY, setting, doc, title, page, convertedImg) {
            var firstData = converSingleTable(null);
            ...
            return renderSinglePage(firstData, firstPageY, doc, title).then(function () {
                ...
                //            doc.addPage(pageIndex++);
                //            return renderSinglePage(converSingleTable(null), setting.yOffset, doc).then(function () {
                //                if (setting.showPageNum) {
