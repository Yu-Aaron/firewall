...
// gulp.task('dist:rev', ['dist:clean'], function () {
    return gulp.src(paths.tmpDir + '**')
        .pipe($.revAll.revision({ dontSearchFile: ['3rd.js'], includeFilesInManifest: ['.css', '.js', '.png'], fileNameManifest: 'js/rev-manifest.json' }))
...
gulp.task('dist:copy', ['dist:linker'], function () {
    return gulp.src([paths.json], {base: paths.srcDir})
	.pipe(gulp.dest(paths.distDir));
});
