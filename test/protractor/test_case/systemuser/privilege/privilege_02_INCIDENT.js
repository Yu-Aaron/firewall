...
describe('New Privilege Test:', function() {
	...
	var privilegeName = "INCIDENT";
	var userName = "INCIDENT_User";
	var groupName = "INCIDENT_GROUP";
	var privilegeGroupName = ["EVENT"];
	...
	it('1. Default admin should have privilege for ' + privilegeName + ': ', function() {
		...
		expect(element(by.id('monitor_incident_incident')).isDisplayed()).toBeTruthy();
	});
	...
	it('3. Change ' + groupName + ' to have view right, user should not be able to view buttons: ', function() {
		...
		//====================Test for shown UI==============================
		element(by.id('header_li_report')).click().then(function () {
			expect(element(by.id('report_event_incident_tab')).isDisplayed()).toBe(true);
			expect(element(by.id('report_event_event_tab')).isDisplayed()).toBe(false);
		});
		element(by.id('header_li_monitor')).click().then(function () {
		...
		element(by.id('monitor_a_event')).click().then(function(){
			expect(element(by.id('monitor_incident_incident')).isDisplayed()).toBe(true);
			expect(element(by.id('monitor_incident_event')).isDisplayed()).toBe(false);
			expect(element(by.id('monitor-event-incident_button_markAllRead')).isDisplayed()).toBe(false);
			expect(element(by.id('monitor-event-incident_button_markAllDeleted')).isDisplayed()).toBe(false);
		});
