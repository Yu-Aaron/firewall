...
describe('New Privilege Test:', function() {
	...
	var privilegeName = "BLACKLIST";
	var userName = "BLACKLIST_User";
	var groupName = "BLACKLIST_GROUP";
	...
	it('1. Default admin should have privilege for ' + privilegeName + ': ', function() {
	    ...
	    expect(element(by.id('rule-blackList-editor_i_movePreDep')).isDisplayed()).toBeTruthy();
	});
	...
	it('2. Create ' + groupName + ' Group and User, user should not be able to view ASSET tab, and MONITOR-DEVICE tab', function() {
	    ...
	    browser.get(host + "/rule/blacklist").then(function () {
	    	browser.getCurrentUrl().then(function(actualUrl) {
	    		console.log("User will be redirected to monitor-overview when try to access to blacklist page by entering url");
	...
	it('3. Change ' + groupName + ' to have view right, user should not be able to view buttons: ', function() {
		...
		element(by.id('header_li_rule')).click().then(function () {
			expect(element(by.id('rule_blackList_noView_noRule')).isDisplayed()).toBe(true);
