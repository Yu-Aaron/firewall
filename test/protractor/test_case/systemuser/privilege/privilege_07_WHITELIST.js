...
describe('New Privilege Test:', function() {
	...
	it('1. Default admin should have privilege for ' + privilegeName + ': ', function() {
	    ...
	    expect(element(by.id('rule-whiteList-editor_i_movePreDep')).isDisplayed()).toBeTruthy();
	});
	...
	it('2. Create ' + groupName + ' Group and User, user should not be able to view ASSET tab, and MONITOR-DEVICE tab', function() {
	    ...
	    browser.get(host + "/rule/whitelist").then(function () {
	    	browser.getCurrentUrl().then(function(actualUrl) {
	    		console.log("User will be redirected to monitor-overview when try to access to whitelist page by entering url");
	...
	it('3. Change ' + groupName + ' to have view right, user should not be able to view buttons: ', function() {
		...
		element(by.id('header_li_rule')).click();
	    element(by.id('rule_a_whitelist')).click().then(function () {
			expect(element(by.id('rule_whiteList_noView_noRule')).isDisplayed()).toBe(true);
			expect(element(by.id('rule-whiteList_button_signatures')).isPresent()).toBe(false);
		});
		login.logout();
